...
class Recognator:
  ...
  def predict(self, file_key):
    ...
    try:
        ...
        #s3.download(file_key, os.getenv('PATH_DOWNLOADED_IMG'))
        print(f"Arquivo baixado com sucesso.")
        prediction = self.match_pet(self.loaded_model, self.class_names_generated)
        return prediction
  ...
  def match_pet(self, model, labels):
